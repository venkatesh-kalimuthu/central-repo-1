...
public class WebClientConfig {
    ...
    //Ford LLM
    @Value("${spring.security.oauth2.client.provider.fordllm-api.token-uri}")
    private String fordLlmApiTokenURI;
    @Value("${spring.security.oauth2.client.registration.fordllm-api.authorization-grant-type}")
    private String fordLlmAuthGrantType;
    @Value("${spring.security.oauth2.client.registration.fordllm-api.client-id}")
    private String fordLlmApiClientID;
    @Value("${spring.security.oauth2.client.registration.fordllm-api.client-secret}")
    private String fordLlmApiClientSecret;
    @Value("${spring.security.oauth2.client.registration.fordllm-api.scope}")
    private String fordLlmApiScope;
    ...
    private  static final String FORDLLM_API = "fordllm-api";
    ...
    ClientRegistrationRepository clientRegistrationRepository() {
        ...
        var fordLlmApi = ClientRegistration.withRegistrationId(FORDLLM_API)
                .scope(fordLlmApiScope)
                .tokenUri(fordLlmApiTokenURI)
                .clientId(fordLlmApiClientID)
                .clientSecret(fordLlmApiClientSecret)
                .authorizationGrantType(new AuthorizationGrantType(fordLlmAuthGrantType))
                .build();

        return new InMemoryClientRegistrationRepository(scoringApiC, ecaApiC, scoringApiCI, ecaApiCI,fordLlmApi);
    }
    ...
    @Bean("fordllm-api")
    public RestClient fordLlmAPIService(OAuth2AuthorizedClientManager authorizedClientManager) {
        var oauthInterceptor = new CustomOAuth2ClientHttpRequestInterceptor(authorizedClientManager, FORDLLM_API);
        return RestClient.builder()
                .requestInterceptor(oauthInterceptor)
                .build();
    }
